...
export default class TeamBalancer extends BasePlugin {
    ...
    async pollLayerInfo() {
        try {
            ...
            if (!layer || !layer.gamemode) {
                this.logDebug('Polling: Layer info is still invalid, continuing to poll.');
                return;
            }

            // Successfully got valid layer info, so cache it and stop polling
            this.logDebug(
                `Polling: Successfully retrieved layer info. Gamemode: ${layer.gamemode}`
            );
            ...
            this.logDebug(
                `Cached info updated. Gamemode: ${this.gameModeCached}, T1: ${this.cachedTeam1Name}, T2: ${this.cachedTeam2Name}`
            );
            this.stopLayerInfoPolling();
        } catch (err) {
            this.logWarning(`Polling: Error fetching layer info:`, err.message);
        }
    }
    ...
    async initiateScramble(isSimulated = false, immediate = false, steamID = null, player = null) {
        ...
        if (!immediate) { // This means it's a live scramble with a countdown
            ...
            return true;
        } else { // This means it's an immediate LIVE scramble (!scramble now)
    ...
    async executeScramble(isSimulated = false, steamID = null, player = null) {
        ...
        try {
            ...
            if (!isSimulated) {
                ...
                this.log.info(msg); // Changed from logDebug to log.info for dry run completion message
            }
            ...
            this.cleanupScrambleTracking();
            this.resetStreak('Scramble execution failed');
            return false;
        } finally {
            this._scrambleInProgress = false;
        ...
        this.scrambleRetryTimer = setInterval(async () => {
            ...
            try {
                ...
                if (this.log && typeof this.log.error === 'function') {
                    ...
                    console.error(`[TeamBalancer] Critical error in scramble retry interval, logger unavailable: ${err.message}`);
                }
    ...
    async processScrambleRetries() {
        ...
        for (const [steamID, moveData] of this.pendingPlayerMoves.entries()) {
            ...
            try {
                        ...
                        if (this.options.warnOnSwap) {
                            try {
                                await this.server.rcon.warn(
                                    steamID,
                                    this.RconMessages.playerScrambledWarning
                                ); // Use the new succinct message
                            } catch (err) {
                ...
                this.log.error(
                    `Error processing move for player ${steamID}: ${iterationErr.message}`
...
const CommandHandlers = {
    register(tb) {
        ...
        tb.RconMessages = {
            ...
            immediateDryRunScramble: 'Manual dry run scramble triggered by admin | Simulating immediate scramble...',
        ...
        tb.onChatMessage = async function (info) {
            ...
            if (this.options.debugLogs) {
                ...
                console.log(`[TeamBalancer] !teambalancer command received from ${playerName} and responded.`);
            }
        ...
        tb.onChatCommand = async function (command) {
            ...
            if (!message.trim()) {
                this.logDebug('No subcommand provided for !teambalancer (admin chat), letting onChatMessage handle public status.');
                return;
            }
            ...
            try {
                switch (subcommand) {
                    ...
                    case 'off': {
                        ...
                        console.log(
                            `[TeamBalancer] Win streak tracking disabled by ${adminName}`
                        );
                    ...
                    case 'status': {
                        ...
                        const lastScrambleTimeFormatted = this.lastScrambleTime
                            ...
                            : 'Never';
                        let currentServerLayer = null;
                        try {
                            currentServerLayer = await this.server.currentLayer;
                        } catch (err) {
                            this.logWarning(`Failed to fetch current layer for diagnostics: ${err.message}`);
                        }
                        const gameModeFromLayer = currentServerLayer?.gamemode || 'N/A';
                        const layerName = currentServerLayer?.layer || 'N/A';
                        const team1NameFromLayer =
                            currentServerLayer?.teams?.[0]?.name || currentServerLayer?.teams?.[0]?.faction || 'N/A';
                        const team2NameFromLayer =
                            currentServerLayer?.teams?.[1]?.name || currentServerLayer?.teams?.[1]?.faction || 'N/A';
                        ...
                        const statusMsg = [
                            `--- TeamBalancer Status for ${adminName} ---`,
                            `Plugin Status: ${effectiveStatus}`,
                            `Win Streak: Team ${this.winStreakTeam ?? 'N/A'} has ${this.winStreakCount} win(s)`,
                            ...
                            `Debug Logging: ${this.options.debugLogs ? 'ON' : 'OFF'}`, // Added debug logging status
                            `Current Layer: ${layerName} (Game Mode: ${gameModeFromLayer})`,
                            `Cached Game Mode: ${cachedGameMode}`,
                            `Team 1 Name: ${team1NameFromLayer} (Cached: ${cachedTeam1Name})`,
                            `Team 2 Name: ${team2NameFromLayer} (Cached: ${cachedTeam2Name})`,
                            `------------------------------------------`
                        ].join('\n');

                        console.log(`[TeamBalancer] Status requested by ${adminName}`);
                    ...
                    case 'scramble': {
                        ...
                        let responseMessage;
                        let immediateExecution = false; // Default to countdown for live

                        if (this.options.dryRunMode) {
                            responseMessage = 'Initiating immediate dry run scramble...';
                            immediateExecution = true; // Force immediate for dry run
                        } else {
                            responseMessage = 'Initiating manual scramble with countdown...';
                            immediateExecution = false; // Keep countdown for live
                        }

                        console.log(`[TeamBalancer] ${adminName} initiated a manual scramble.`);
                        this.respond(player, responseMessage);

                        const success = await this.initiateScramble(
                            this.options.dryRunMode, // isSimulated
                    ...
                    case 'diag': {
                        this.logDebug('Diagnostics command received.'); // Removed redundant prefix
        ...
        tb.onScrambleCommand = async function (command) {
            ...
            try {
                if (subcommand === 'now') {
                    ...
                    // Differentiate immediate scramble messages
                    const responseMessage = this.options.dryRunMode
                ...
                } else { // This is for !scramble (without 'now')
                    if (this._scramblePending || this._scrambleInProgress) {
                        const status = this._scrambleInProgress ? 'executing' : 'pending';
                        this.respond(
                            player,
...
export const Scrambler = {
    async scrambleTeamsPreservingSquads({
        ...
        log(
            `Total players: ${totalPlayers}, Max per team: ${maxTeamSize}, Scramble Percentage: ${scramblePercentage * 100
            }%`
        );
